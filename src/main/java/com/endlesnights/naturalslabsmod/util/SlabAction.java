...
public class SlabAction 
{
            ...
            PlayerEntity playerIn, Hand hand, BlockRayTraceResult p_225533_6_)
    {
		...
		{
			BlockState blockstate = ModBlocks.block_farmland_slab.getDefaultState().with(SlabBlock.TYPE, state.get(SlabBlock.TYPE)).with(SlabBlock.WATERLOGGED, state.get(SlabBlock.WATERLOGGED));
            ...
            PlayerEntity playerIn, Hand hand, BlockRayTraceResult p_225533_6_)
    {
		...
		if (context.getFace() != Direction.DOWN && world.getBlockState(blockpos.up()).isAir())
		{
		    BlockState blockstate = ModBlocks.block_path_slab.getDefaultState().with(SlabBlock.TYPE, state.get(SlabBlock.TYPE)).with(SlabBlock.WATERLOGGED, state.get(SlabBlock.WATERLOGGED));
